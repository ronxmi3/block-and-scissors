    ...
    <main className="site-shell">
      ...
      <section className="workspace">
        ...
        <article className="panel escrow-panel">
          ...
          <div className={`escrow-state ${escrowId ? 'ready' : ''}`}>
            ...
            <div>
              <strong>{escrowId ? 'Escrow locked' : 'Waiting for escrow'}</strong>
              <p>{escrowId ? 'Ready for AI verification.' : 'Create one or paste an existing Testnet object ID.'}</p>
            </div>
          </div>
      ...
      <section className="verify-section">
        ...
        <div className="verify-action-row">
          ...
          <button className="verify-button" type="button" onClick={verifyHaircut} disabled={verifying || !reference || !result || !escrowId}>
            {verifying ? <Loader2 className="spin" size={18} /> : <Sparkles size={18} />}
            {verifying ? 'Comparing haircuts…' : dryRun ? 'Run AI simulation' : 'Verify & settle'}
          </button>
        </div>
